...
public class MinnBotUserInterface extends JFrame {
	...
	private static PopupErrorLog popupErrorLog;
	...
	public MinnBotUserInterface() {
		popupErrorLog = new PopupErrorLog();
		...
		JCheckBox chckbxLogErrors = new JCheckBox("Log Errors");
		...
		btnAccount.addActionListener(e -> {
            if (as != null) {
                ...
                writeError("[MINNBOT] Bot must be launched to access account settings.");
            }
        });
		...
		btnStop.addActionListener(e -> {
            try {
                ...
                if (api != null) {
                    ...
                    writeError("[ERROR] MinnBot is not currently running.");
                }
            } catch (Exception ex) {
                writeError("[ERROR] Shutdown failed: \n> " + ex.getMessage());
            }
        });
		...
		errorArea.setBounds(10, 258, 493, 87);
		panel.add(errorArea);
		...
		errorArea.setText("Error Logs");
		errorArea.setFont(new Font("Century", Font.BOLD, 10));
            ...
            if (bot.api != null) {
                if (sLog == null) {
                    try {
                        ...
                        writeError("[MINNBOT] Exception - " + e1.getMessage());
                        return;
                    }
                ...
                writeError("[MINNBOT] You can not access this window without loggin in.");
            }
        });
		...
		btnRestart.addActionListener(e -> {
            ...
            try {
                ...
                e1.printStackTrace();
                writeError("[MINNBOT] " + e1.getMessage());
            }
        });
		btnRestart.setEnabled(false);
